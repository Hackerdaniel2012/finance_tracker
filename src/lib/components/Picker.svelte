...
<div class={`relative ${className}`} bind:this={container}>
	<button
		class="flex h-11 w-full items-center justify-between gap-3 rounded-ui border border-zinc-300 bg-white px-5 py-2 text-left text-base font-semibold text-zinc-950 transition focus:border-blue-600 focus:outline-2 focus:outline-offset-2 focus:outline-blue-500/20 disabled:cursor-not-allowed disabled:opacity-50"
		type="button"
	...
	{#if open}
		<div
			class={`absolute left-0 z-50 w-full overflow-hidden rounded-ui border border-zinc-200 bg-white ${menuPlacement === 'above' ? 'bottom-[calc(100%+0.5rem)]' : 'top-[calc(100%+0.5rem)]'}`}
